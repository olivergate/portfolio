...
- **`.pipeline-row[data-state="done"]` 0.5 opacity may fail WCAG AA.** Green-glyph + label at 0.5 opacity over `--terminal` (#0f0e0b) is borderline against `--terminal-fg` (#e8e2d0) for non-text contrast (3:1 minimum). Cheap fix: bump to 0.65. (Selector renamed from `.lab-pipeline-row` and moved to `styles/globals.css` when the pipeline was shared with `/jd` — ADR-0042.)
